...
First, we extract the aAICic values of each fit.
```{r aic_extract}
kable(
  t(data.frame(
    AR1 = round(ar1$aic, 3),
...
$$
Where the $p$_value is found via the $\chi^2$ distribution and the test statistic, given by the following for a pure AR model (i.e. $q = 0$)
...
Now, we the `tsdiag` function in R to plot diagnostics for your two models.
...
We see that the AR$(1)$ model has Ljung-Box $p$-values below the $\alpha = 0.05$ significance threshold $\forall h > 1$, which is confirmed by out `lb_stats` object, as you can see below:
```{r}
which(lb_stats[, 1] < 0.05)
```
This tell us that we reject $H_0$ in favour of the alternative that the model is ill-fitting for the AR$(1)$ case in the majority of scenarios. This implies that it may be likely that the AR$(1)$ model is not well-specified for these data. This theory is supported by the ACF of the residuals, which appears to have a vague sinusoidal pattern to it, as well as having values above $\pm 2 / \sqrt{n}$ for quite a few of the lags. All in all, the `tsdiag` results for the AR$(1)$ model shed doubt on our confidence of this candidate being well-fitting for these data.
...
We again let $m \in M$ be the candidate models, for $M = \{\text{AR}(1), \text{AR}(6)\}$. We let $\{y_t\}_{t \in \mathbb{Z}}$  be the observed values from the test set. We let $\hat{y}^{(m)}_t(\ell)$ be the $\ell$-step ahead Box-Jenkins forecast (for $\ell \in \mathbb{Z}$) of model $m$. Let $N_{\ell}$ be the total number of steps ahead. In our case, this is $23$ for both.
...
$$
```{r}
N_L = length(nino_test)
# make predictions for held-out test data
ar1_forecast = predict(ar1, n.ahead = N_L)
ar6_forecast = predict(ar6, n.ahead = N_L)
mods = c("AR(1)", "AR(6)")
# compute MSPE for each
mspes = sapply(list(ar1_forecast, ar6_forecast),
               function(m){
                 mean( (nino_test - m$pred)^2 )
               })
# report results
...
$$
m_{\text{best}} = \underset{m \in M}{\text{argmin}}\Big\{\text{MSPE}(m)\Big\} = \underset{m \in M}{\text{argmin}}\Big\{ 1.200, 1.016\Big\} = \text{AR}(6)
$$
...
First, we compute the $95\%$ prediction intervals using the following method: As before, we let $\hat{y}^{(m)}_t({\ell})$ be the $\ell \in \mathbb{Z}$-step ahead Box-Jenkins forecast for model $m \in M$. We let $\text{Z} \sim N(0,1)$ and "PrI" indicate the prediction interval at step $\ell$, given significance level $\alpha$ and model $m$.
$$
...
ar1_Low = ar1_forecast$pred - Z*ar1_forecast$se
ar1_High = ar1_forecast$pred + Z*ar1_forecast$se
# prediction intervals for AR(6)
ar6_Low = ar6_forecast$pred - Z*ar6_forecast$se
ar6_High = ar6_forecast$pred + Z*ar6_forecast$se
# construct the data frame
bigplot_DF = data.frame(
  Time = as.Date(time(nino_test)),
  Observed = as.numeric(nino_test),
  Forecast_AR1 = as.numeric(ar1_forecast$pred),
  Lower_AR1 = as.numeric(ar1_Low),
  Upper_AR1 = as.numeric(ar1_High),
  Forecast_AR6 = as.numeric(ar6_forecast$pred),
...
pfancy = ggplot(data = bigplot_DF, aes(x = Time)) +
  ...
  geom_ribbon(aes(ymin = Lower_AR1, ymax = Upper_AR1), fill = "#caf0f8", alpha = 0.15) +
  geom_ribbon(aes(ymin = Lower_AR6, ymax = Upper_AR6), fill = "#ff9991", alpha = 0.15) +
  # legend and colour assignment
  scale_color_manual(name = "",
    values = c(
    "Actual" = "#696969",
    "Forecast AR(1)" = "#02a9ea",
    "95% Interval AR(1)" = "#9fc8d6",
    "Forecast AR(6)" = "#ff2e1f",
    "95% Interval AR(6)" = "#fc796f"
  )) +
...
Then we find a logical combination of $p$ and $P$, we can informally examine the PACF for what seems "sensible." What immediately jumps out to me is the presence of a large $\hat\alpha_{kk}$ at $k = 12$. This is indicative of a seasonal AR component, i.e. $P \geq 1$. Further, there is an immediate jump at $\hat\alpha_{11}$ as well as other "spikes" across the PACF at $\hat\alpha_{11, 11},  \hat\alpha_{23, 23}$, etc. Since these values $\{1, 11, 23, \dots \}$ are not evenly divisible by $12$, we know that there must exist one or more sequential AR components, i.e. $p \geq 1$. For the sake of parsimony, my initial suggestion would be to choose $P = p = 1$ as a starting point. 

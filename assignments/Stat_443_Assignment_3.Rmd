...
print(p2f)
```
**Comments**: While there's no noticeable long-term pattern to the residuals as we would hope if the model is well fitting, there is a sharp increase in the residual values from 2019 onwards. This could be due to the large fluctuations in working hours caused by the COVID-19 pandemic. However, beyond these extreme values, the residuals appear to be randomly distributed about zero, as we would anticipate for a well-fitting model.
...
```{r}
n = length(hours_train); lagmax = 35
# get rho(h) values of residuals
...
# these include but are not limited to the high standardized residuals for 2019-2020, the large residual ACF values at certain lags
# and the sudden "shock" experienced in the Ljung-Box Portmanteau p-values every 12 lags starting at h = 6. while we fail to reject

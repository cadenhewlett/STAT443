...
$$X_t = a_0 + a_t \cos(\omega_p \cdot t) + b_t \sin(\omega_p \cdot t) + \varepsilon, \text{ where } \varepsilon \overset{\text{iid}}{\sim}N(0, \sigma^2_{\varepsilon}) \text{ and } t \in \{1, 2, \dots, N\}$$

We can equivalently formulate this model as:
$$
X^{\,p}_t = a_0 + \sum_{t = 1}^N \Big(a_t \cos(\omega_p \cdot t) + b_t \sin(\omega_p \cdot t) \Big) + \varepsilon, \text{ where } \varepsilon \overset{\text{iid}}{\sim}N(0, \sigma^2_{\varepsilon})
$$
Where the superscript $p$ indicates the Fourier Frequency $\omega_p \in \Omega$.

Hence, the $p$-th model fitted will have the form:
$$
\widehat{X}^{\,p}_t = \hat{a}_0 + \sum_{t = 1}^N \Big(\hat{a}_t \cos(\omega_p \cdot t) + \widehat{b}_t \sin(\omega_p \cdot t) \Big), \text{ where } \omega_p \in \Omega
$$
...
$$
\mathcal{F} : \Big\{F\big(\,\widehat{X}^{\,\underset{p}{}}_t\,\big) : \widehat{X}^{\,\underset{p}{}}_t\in {\mathfrak{X}}\Big\}
$$

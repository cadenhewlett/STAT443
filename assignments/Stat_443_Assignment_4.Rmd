...
**Comments**: Dominated by low frequencies, two peaks in spectrum magnitude at (...) and (...), tails off into noise as $f \uparrow$, for higher frequencies. Thereby we know a (... larger proportion of the total variation in the series is attributed to the lower frequencies in the series (?).) There may also be signs of harmonics, as we can see roughly equidistant peaks at (... values go here for those first three after big peak)
...
dfreq
```
The the Angular Frequency from can be found by converting back to its corresponding Fourier Frequency (in radians.)
$$
...
Document the inputs and outputs of this function so that another person would be able to understand how to use your function. 

The full function definition is below:
```{r}
...
When $p=10$, the function return $\pi/5$.
```{r unit_tests}
...
Here, we will fit all the fourier frequencies and determine which are significant, do some testing, and fit a finalized model.


### Part D.1. 
In order to do this, we will first compute the set of frequencies for $p \in \{0, 1, \dots N/2\}$ using the function from the previous question.

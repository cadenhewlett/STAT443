...
```{r, include=FALSE}
p1c2 = ggplot(data = p1c2data, aes(x = Time)) +
    # fill
    geom_ribbon(aes(ymin = Lower_AR6, ymax = Upper_AR6), fill = "#fa98ac", alpha = 0.15) +
    geom_ribbon(aes(ymin = Lower_HW,  ymax = Upper_HW), fill = "#FFEE70", alpha = 0.15) +
    # lines for obsv, pred and interval bounds
    geom_line(aes(y = Observed, color = "Actual"),  lwd = 1) +
    geom_line(aes(y = Forecast_HW, color = "Holt-Winters Forecast"),lwd = 1) +
    geom_line(aes(y = Forecast_AR6, color = "Box-Jenkins Forecast"),lwd = 1) +
    # points, too
    geom_point(aes(y = Observed, color = "Actual")) +
    geom_point(aes(y = Forecast_HW, color = "Holt-Winters Forecast")) +
    geom_point(aes(y = Forecast_AR6, color = "Box-Jenkins Forecast")) +
    # confidence intervals
    geom_line(aes(y = Lower_HW, color = "95% HW Prediction Interval"), alpha = 0.75) +
    geom_line(aes(y = Upper_HW, color = "95% HW Prediction Interval"), alpha = 0.75) +
    geom_line(aes(y = Lower_AR6, color = "95% BJ Prediction Interval"), alpha = 0.75) +
    geom_line(aes(y = Upper_AR6, color = "95% BJ Prediction Interval"), alpha = 0.75) +
    # legend and colour assignment
    scale_color_manual(name = "",
      values = c(
      "Actual" = "#250902",
      "Holt-Winters Forecast" = "#f7b801",
      "95% HW Prediction Interval" = "#FFEE70",
      "Box-Jenkins Forecast" = "#db2146",
      "95% BJ Prediction Interval" = "#fa98ac"
    )) +
    # customize x-axis for nice dates
    scale_x_date(date_breaks = "4 month",  date_labels = "%b, %Y") +
    # add titles with units
    labs(
    title = "Forecasts of El Nino 3.4 Index from Jan. 2022 to Nov. 2023",
    subtitle = expression(
      paste("Index Represents Average Equatorial Sea Surface Temperature (in ", 
          degree, "C)")),
      x = "Month and Year",
...
We construct the $95\%$ prediction intervals and forecasts by the same method discussed in detail in the previous question. In this case, our largest value ahead is $36$, so we make forecasts $\forall \ell \in \{1, 2, \dots, 36\}\subset \mathbb{Z}$.
```{r}
# declare constants
alpha = 0.05
Z = qnorm(1-alpha/2)
...
\text{Forecast: }&  \hat x_{t}(\ell) = L_t + \ell \, T_t + I_{t - p + \ell}\\
...
Let $x_t$ be the true value of the test time series at time $t$, for $t \in \{(t+1), (t + 2), \dots, (t + \varkappa) \}$ (assuming the training set ends at time $t$ and is contiguous with the test set), where $\varkappa \in \mathbb{N}$ is the lead at the end of the test set, i.e. $\ell \in [1, \varkappa]$ for the $\ell$-step ahead forecast methods. In our case, $\varkappa = 36 = |\text{Test Set}|$ as we have 36 predictions. 


Similarly, we let $\hat{x}^{\text{H}}_t(\ell)$ be the $\ell$-step ahead Holt-Winters forecast and $\hat{x}^{\text{B}}_t(\ell)$ be the Box-Jenkins forecasted value at time $t + \ell$.

Then, the MSPE of the two models are given by the set:

$$
\text{MSPE} = \bigg\{\text{M} \in \{\text{H}, \text{B}\}:\frac{1}{\varkappa}\sum_{\ell = 1}^{\varkappa}\big(\,x_{t+\ell} - \hat{x}^{\text{M}}_t(\ell)\, \big)^2 \bigg\}
$$
```{r}
...
$$
m_{\text{best}} = \underset{\text{M} \in  \,\{\text{H}, \text{B}\}}{\text{argmin}}\big\{\text{MSPE}(\text{M})\big\} = \underset{\text{M} \in  \,\{\text{H}, \text{B}\}}{\text{argmin}}\big\{0.0437,	0.0351\big\} = \text{B}
$$

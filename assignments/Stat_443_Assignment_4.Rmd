...
Below, we plot $X_t$ alongside $\widehat{X}_{\text{unified}}$ for the training data. We also add a $95\%$ confidence interval for the in-sample fit, to have a rough quantification of the uncertainty inherent in the fitted unified model.
```{r}
...
p7DF = data.frame(
  ...
  Upper = as.numeric(model_fit$lwr)
)
# create plot
p7 = ggplot(data = p7DF, aes(x = Time)) +
  # lines for obsv, pred and interval bounds
  geom_line(aes(y = Actual, color = "Actual"),  lwd = 0.8, alpha = 0.7) +
  geom_line(aes(y = Predicted, color = "Fitted"), lwd = 0.8, alpha = 0.7) +

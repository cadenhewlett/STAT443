...
\end{aligned}
$$
Then, we can evaluate at our given $\alpha_1$ and $\alpha_2$, simplifying the fraction above using Python to avoid human error.
```{python}
from sympy import *
...
cat("Variance of Simulated ARIMA process:", simulated_gamma_0, "\n")
```
...
The normalized spectral density ended up being quite long, so we'll define each term one-by-one in the function below:
```{r}
f = function(w){
  term_1 = 1
...
test = seq(from = 0, to = 8*pi, length.out = 1000)
plot(test, f(test), type = 'l')
```

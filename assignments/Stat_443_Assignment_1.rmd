...
Recall that, for a time series of length $n$, from a white noise process, the sample autocorrelation coefficient at lag $h$
approximately follows a normal distribution with mean $-1/n$ and variance $1/n$:
$$r_h \sim N (-1/n, 1/n)$$
for large values of $n$. To confirm this theoretical fact, we'll conduct a simulation study for lags $h = 1$ and $h = 2$.
...
We know that the theoretical approximation should obey the following distribution:

$$
\forall h, \; r_h \sim N\Big(\mu = -\frac{1}{n}, \sigma^2 = \frac{1}{n} \Big)
$$
From our empirical observations based on the plots in the previous question, it seems likely that both the true $r_1$ and $r_2$ values follow the theoretical approximation.

To analytically verify these claims, we will construct confidence intervals for the mean and variances at $\alpha = 0.05$, i.e. $95\%$. We will begin by constructing pairwise confidence intervals for the means of $r_1$ and $r_2$ to determine if $-1/n$ is in these intervals. To adjust for family-wise error rate, we'll apply the Bonferroni correction and use $\alpha' = 0.05/2$, in other words constructing $97.5\%$ intervals. 
...
$$\forall h \in [1, 2], \;\text{bias}(r_h) = \mathbb{E}(\mathcal{R}_h) - \Big(-\frac{1}{n}\Big) \approx 0$$
...
Putting all this together, the consistency of the confidence intervals with the theoretical approximations give us some important analytical credence to our initial empirical observations. We can conclude that it is overwhelmingly likely true that the simulated 

...
\end{aligned}
$$
Then, we can evaluate at our given $\alpha_1$ and $\alpha_2$, simplifying the fraction using Python.
```{python}
from sympy import *
...
```
Then, we can verify both our results and Python's simplification as follows:
```{r}
set.seed(3)
alpha_1 <- 7/10
alpha_2 <- -1/10
...
cat("Computed Variance Using Computation:", 275/162, "\n")

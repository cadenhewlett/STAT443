...
## Qualitative ##
# NOTE: HW appears quite close to the true values for \ell small, but becomes worse as time progresses.
# Further, it begins to have a much wider prediction interval (i.e. become more uncertain) as the steps ahead increase
# While the Box-Jenkins methods also exhibit this increase in uncertainty, it is less extreme (noting that the lower end 
# of the Holt-Winter PrI is near 22, which is completely outside of the range of the BJ approach)

# when comparing to the squared prediction error to that of the
# better-performing AR(6) Box-Jenkins model, we see for smaller values 
# of \ell there are actually cases in which the HW-Model has a smaller
# squared prediction error. However, as the plot below demonstrates, 
# that error skyrockets as the prediction distance increases. 
# while the Box-Jenkins model also has a sharp squared error increase, 
# it is much smaller than that of the Holt-Winters model
err_HW = (nino_test - hw_predictions$fit )^2
err_BJ = (nino_test - ar6_fit)^2
plot(err_HW, ylim = c(-1, 10))
lines(err_BJ, col = 'red')

## Quantitative ##
## HW has higher mean Squared prediction Error
```

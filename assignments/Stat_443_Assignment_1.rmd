...
# lag on 1 year : doesn't look like there's much serial dependence (we'd see a more obvious pattern to the correlogram), this mostly appears to be white noise
acf(deseasonalized, na.action = na.pass, lag.max = 12)

# lag on many years: the acf appears to still basically be white noise, the magnitude of which seems to be declining over time, as we'd expect
...
IID noise: the sample acf tends to be within $\pm 2 / \sqrt{n}$ bounds.
```{r noisytest}
